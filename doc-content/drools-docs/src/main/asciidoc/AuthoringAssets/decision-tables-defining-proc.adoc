...
Only the first worksheet in a spreadsheet workbook is processed as a decision table when you upload the spreadsheet in {CENTRAL}. Each `RuleSet` name combined with the `RuleTable` name must be unique across all decision table files in the same package.

If you want to process multiple worksheet decision tables, then create a `.properties` file with the same name as the spreadsheet workbook. The `.properties` file must contain a property sheet including the comma-separated names of the worksheets as follows:
