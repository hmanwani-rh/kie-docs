...
NOTE: When you use {PRODUCT} without an executable rule model, the consequences that are written using `dialect "JAVA"` are supported until JAVA 5 version. For more information about executable rule models, see
ifdef::DM,PAM[]
