...
Enable lambda externalization which optimizes the memory consumption at runtime. It rewrites lambdas that are generated and used in the executable model, enabling you to reuse the same lambda multiple times with all the patterns and the same constraint. The executable model becomes garbage collectible once the rete and phreak is instantiated.
+
--
Use the following property to enable lambda externalization for the executable model:
