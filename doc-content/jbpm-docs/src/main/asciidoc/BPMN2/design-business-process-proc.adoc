...
+
NOTE: A task, event, or gateway in {PRODUCT} expects one incoming and one outgoing flow. If you want to design a business process with multiple incoming and multiple outgoing flows, set the system property `jbpm.enable.multi.con` to `true`. 
