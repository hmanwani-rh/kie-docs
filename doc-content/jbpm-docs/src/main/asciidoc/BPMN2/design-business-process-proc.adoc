...
[NOTE]
====
A task and event in {PRODUCT} expects one incoming and one outgoing flow. If you want to design a business process with multiple incoming and multiple outgoing flows, use gateways (recommended) or set the JVM system property `jbpm.enable.multi.con` to `true`.
