...
However, if it is a must to use multiple connections for a task or an event, then you must set the JVM (Java virtual machine) system property `jbpm.enable.multi.con` to `true`. When `jbpm.enable.multi.con` property is enabled for JVM, then ensure that the business process that JVM executes contains the similar system property as enabled otherwise, the process engine throws an exception.
====
