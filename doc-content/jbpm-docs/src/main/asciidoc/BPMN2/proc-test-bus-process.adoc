...
|`tearDown`
|This method is annotated as `@After`. It removes history, closes `EntityManagerFactory` and a data source, and disposes of `RuntimeManager` and `RuntimeEngines`.

|===
--

* Knowledge base and knowledge session management methods: To create a session, create `RuntimeManager` and `RuntimeEngine`. Use the following methods to create and dispose of `RuntimeManager`:
+
--
